...
function _initMedia(container) {
  ...
  container.innerHTML =
    '<div class="sm-track" id="smTrack">' +
      MEDIA_FILES.map(function (u, i) {
        var isVideo = u.toLowerCase().endsWith(".mp4");
        var activeClass = i === 0 ? "active" : "";
        
        // Для відео додаємо autoplay, loop, muted та playsinline, щоб воно працювало як GIF
        var mediaHtml = isVideo 
          ? '<video src="' + u + '" class="sm-img" autoplay loop muted playsinline ' +
            'onloadeddata="this.parentNode.classList.add(\'loaded\')" ' +
            'onerror="this.parentNode.classList.add(\'error\')"></video>'
          : '<img src="' + u + '" alt="" class="sm-img" draggable="false" ' +
            'onload="this.parentNode.classList.add(\'loaded\')" ' +
            'onerror="this.parentNode.classList.add(\'error\')" />';

        return '<div class="sm-frame ' + activeClass + '">' +
  ...
  // Обробка жестів та кліків залишається без змін
