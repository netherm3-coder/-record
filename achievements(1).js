...
const MILESTONES = [
  ...
  // === ЖИМ ЛЕЖАЧИ ===
  { id: "bench_100", name: "Прощавай, пустий гриф", description: "Жим лежачи 100 кг", icon: "🏋️", category: "strength", reward: 40,
    condition: (w) => maxWeightFor(w, "Жим лежачи") >= 100,
    progress: (w) => Math.min(1, maxWeightFor(w, "Жим лежачи") / 100) },

  { id: "bench_150", name: "Гриф починає гнутися", description: "Жим лежачи 150 кг", icon: "💥", category: "strength", reward: 80,
    condition: (w) => maxWeightFor(w, "Жим лежачи") >= 150,
    progress: (w) => Math.min(1, maxWeightFor(w, "Жим лежачи") / 150) },

  { id: "bench_200", name: "Прощавай, легка атлетико", description: "Жим лежачи 200 кг", icon: "🦍", category: "strength", reward: 150,
    condition: (w) => maxWeightFor(w, "Жим лежачи") >= 200,
    progress: (w) => Math.min(1, maxWeightFor(w, "Жим лежачи") / 200) },

  // === МЕТА (рахується від суми зароблених очок) ===
...
function maxWeightFor(workouts, exName) {
  ...
  let max = 0;
  (workouts || []).forEach((w) => {
    if (!w.exercise || !String(w.exercise).toLowerCase().includes(needle)) return;
    const m = String(w.count).match(/\(\s*\+?\s*([\d.]+)\s*кг\s*\)/i);
    if (m) { const v = parseFloat(m[1]); if (v > max) max = v; }
  });
  return max;
}
...
// ================================================================
//  РІВНІ (як у PUBG): споріднені досягнення — одна картка з рівнями
// ================================================================
const TIER_GROUPS = {
  pullups:     { title: "Підтягування",   ids: ["first_pullup", "ten_pullups", "twenty_pullups", "fifty_pullups", "century_pullups"] },
  bench:       { title: "Жим лежачи",     ids: ["bench_100", "bench_150", "bench_200"] },
  sober:       { title: "Тверезість",     ids: ["sober_7", "sober_30", "sober_100", "sober_365"] },
  volume:      { title: "Обсяг записів",  ids: ["first_step", "collector", "archivist"] },
  streak:      { title: "Серія днів",     ids: ["iron_will", "marathon_30"] },
...
// id -> { group, level, total }
const TIER_OF = {};
Object.keys(TIER_GROUPS).forEach((g) => {
  TIER_GROUPS[g].ids.forEach((id, i) => {
    TIER_OF[id] = { group: g, level: i + 1, total: TIER_GROUPS[g].ids.length };
  });
});
...
window.computeAchievements = (workouts) => {
  ...
  return MILESTONES.map((m) => {
    const r = m.meta
      ...
      : base[m.id];
    return { ...m, isUnlocked: r.isUnlocked, progress: r.progress, tier: TIER_OF[m.id] || null };
  });
};
...
window.renderAchievementsTab = () => {
  ...
  const cards = [];       // { sortKey, unlocked, html }
  const doneGroups = {};

  list.forEach((a) => {
    if (a.tier) {
      const g = a.tier.group;
      if (doneGroups[g]) return;
      doneGroups[g] = true;

      const ids = TIER_GROUPS[g].ids.filter((id) => byId[id]);
      const steps = ids.map((id) => byId[id]);
      const unlockedSteps = steps.filter((x) => x.isUnlocked);
      const level = unlockedSteps.length;
      const next = steps[level] || null;
      const earned = unlockedSteps.reduce((s, x) => s + x.reward, 0);
      const maxG = steps.reduce((s, x) => s + x.reward, 0);
      const shown = level > 0 ? steps[level - 1] : steps[0];
      const pct = next ? Math.round(next.progress * 100) : 100;
      const isMax = level === steps.length;

      const dots = steps.map((x, i) =>
        '<span class="ach-pip' + (i < level ? " on" : "") + '"></span>').join("");

      cards.push({
        unlocked: level > 0,
        sortKey: level > 0 ? -level : 1 - (next ? next.progress : 0),
        html:
          '<div class="ach-card ach-tier ' + (level > 0 ? "unlocked" : "locked") + (isMax ? " ach-max" : "") + '">' +
            '<div class="ach-tier-badge">' + (level > 0 ? ROMAN[level - 1] : "—") + '</div>' +
            '<div class="ach-icon' + (level > 0 ? "" : " ach-locked-icon") + '">' + (level > 0 ? shown.icon : "🔒") + '</div>' +
            '<div class="ach-name">' + shown.name + '</div>' +
            '<div class="ach-desc">' + (next ? next.description : shown.description) + '</div>' +
            '<div class="ach-pips">' + dots + '</div>' +
            (next
              ? '<div class="ach-progress">' +
                  '<div class="ach-progress-bar"><div class="ach-progress-fill" style="width:' + pct + '%"></div></div>' +
                  '<div class="ach-progress-text">' + pct + '% до ' + ROMAN[level] + '</div>' +
                '</div>'
              : '<div class="ach-status">✓ Максимум</div>') +
            '<div class="ach-reward' + (level > 0 ? "" : " locked-reward") + '">' + earned + ' / ' + maxG + ' очок</div>' +
          '</div>',
      });
      return;
    }
  ...
  container.innerHTML =
    '<div class="ach-header">' +
      '<div class="ach-stats-row">' +
        '<div class="ach-stat-item"><div class="ach-stat-num">' + unlockedCount + "/" + achievements.length + '</div><div class="ach-stat-lbl">розблоковано</div></div>' +
        '<div class="ach-stat-item"><div class="ach-stat-num">' + totalReward + '</div><div class="ach-stat-lbl">очок з ' + maxReward + '</div></div>' +
      '</div>' +
      '<div class="ach-overall-bar"><div class="ach-overall-fill" style="width:' + (unlockedCount / achievements.length * 100) + '%"></div></div>' +
    '</div>' +
    '<div class="ach-filters">' + filterBtns + '</div>' +
    '<div class="ach-grid">' + cards.map((c) => c.html).join("") + '</div>';
